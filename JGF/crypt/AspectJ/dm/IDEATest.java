...
class IDEATest
{
	...
	{
		cipher_idea(0, plain1.length,8, plain1, crypt1, Z);     // Encrypt plain1.
		cipher_idea(0, crypt1.length,8,crypt1, plain2, DK);    // Decrypt.
	}
	...
	/** Bruno Medeiros.
	 * 
	 *  Distributed Memory design rules to apply the local/global view creation
	 * 
	 */
	private byte [] createPlain1() {return new byte [array_rows];}
	private byte [] createCrypt1() {return new byte [array_rows];}
	private byte [] createPlain2() {return new byte [array_rows];}
	...
	/**
	 * Bruno Medeiros : DM design rule to apply the global view
	 */
	private void fillPlain1() 
	{
	...
	/**
	 * Bruno Medeiros: Application of the parallel for design rule valid for both SM and DM
	 * environments;
	 * 
	 * 
	 * @param begin : for begin
	 * @param end	: for end
	 * @param step	: for step
	 * @param text1
	 * @param text2
	 * @param key
	 * 
	 * collateral design rule:
	 * i1 = i2 = i; 
	 * so that chunks can work correctly. since i1 is increment during the for loop body
	 */

	private void cipher_idea(final int begin, final int end, final int step, byte [] text1, byte [] text2, int [] key)

	{
		int i1 = begin;                 // Index into first text array.
		int i2 = begin;                 // Index into second text array.
