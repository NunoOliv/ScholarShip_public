...
void Do() throws MPIException
{
	...
	final boolean masterProcess 	= processID == 0;
	long begin = 0;

	MPI.COMM_WORLD.barrier();
  
	if(masterProcess) // Start the stopwatch. 
	{
		begin = System.currentTimeMillis(); 
	}
    	
	scattering(processID, numberProcesses, plain1, p_plain1);
	
	cipher_idea(p_plain1, p_crypt1, Z);     // Encrypt plain1.
	cipher_idea(p_crypt1, p_plain2, DK);     // Decrypt.

	gathering(processID, numberProcesses, p_plain2, plain2);
	
	if(masterProcess) // Stop the stopwatch.
	{
...
private void scattering(	final int processID, final int numberProcesses, 
						final byte sendB[], final byte recvB[]) throws MPIException
{	 // master process
	 if(processID == 0)
	 {
		 for(int i = 0; i < recvB.length; i++)	// Internal Master copy
		 {
		 	recvB[i] = sendB[i];
		 }
		 
		 final int blocklen[] 	= {1, 0};
		 final int disp[] 		= {0, 0};
		 	
		 for(int slaveID = 1; slaveID < numberProcesses; slaveID++)
		 {	   
			
			 final int iterationsPerWorker = (slaveID==numberProcesses-1) ? rem_p_array_rows : p_array_rows;
			
			 blocklen[1] 	= iterationsPerWorker - 1;
			 disp[1] 		= slaveID * iterationsPerWorker + 1;
			    
			 final Datatype mytype = Datatype.createIndexed(blocklen, disp, MPI.BYTE);
			 mytype.commit();
	
			 final byte saveFirst = sendB[0];
			 sendB[0] = sendB[disp[1]-1];			// swap the first value to send to beginning
						
			 MPI.COMM_WORLD.send(sendB, 1, mytype, slaveID, slaveID);
			 sendB[0] = saveFirst; 			 // restore the original value
		}
	  }
	  else // Slaves
	  {
	    	MPI.COMM_WORLD.recv		(recvB, recvB.length, MPI.BYTE, 0, processID);
	  }
 }


private void gathering(	final int processID, final int numberProcesses, 
						final byte sendB[], final byte recvB[]) throws MPIException
{	 
	// Master process
	if(processID == 0)
	{
		for(int i = 0; i < sendB.length; i++) // Internal Master copy
		{
			recvB[i] = sendB[i];
		}
		...
		{	   
			...
			final byte saveFirst = recvB[0];
					
			MPI.COMM_WORLD.recv(recvB, 1, mytype, slaveID, slaveID);
		
			recvB[disp[1]-1] = recvB[0];					// swap the first value to send to beginning
			recvB[0] 		= saveFirst; 				// restore the original value
			
		}
	}
	else // Slaves
	{
		MPI.COMM_WORLD.send		(sendB, sendB.length, MPI.BYTE, 0, processID);
	}
}
