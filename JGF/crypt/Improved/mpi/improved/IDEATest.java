...
void Do() throws MPIException
{
	...
	final int processID 			= MPI.COMM_WORLD.getRank();
	final int numberProcesses	= MPI.COMM_WORLD.getSize();
	
	// Scattering the plain1 across the slaves into p_plain1
	scattering(processID, numberProcesses);
	
	cipher_idea(p_plain1, p_crypt1, Z);		// Encrypt plain1.
	cipher_idea(p_crypt1, p_plain2, DK);		// Decrypt.

	// Gathering from the slaves p_plain2 into the master plain2
	gathering(processID, numberProcesses);
}

	private void scattering(	final int processID, final int numberProcesses) throws MPIException
	{	 
		// master process
		if(processID == 0)
		{
			...
			final byte saveFirst = plain1[0];	
	
			// Send data to the slaves
			for(int slaveID = 1; slaveID < numberProcesses-1; slaveID++)
			{	   
				sendSpecialType(slaveID, p_array_rows, slaveID * p_array_rows);
			}
			
			// Dealing with the last process (might have different chunk of data)
			if(numberProcesses > 1)
			{
				sendSpecialType(numberProcesses-1, rem_p_array_rows, (numberProcesses-1) * p_array_rows);
			}
		 
			plain1[0] = saveFirst; 				 // restore the original value
		}
		else // Slaves
		{
			MPI.COMM_WORLD.recv		(p_plain1, p_array_rows, MPI.BYTE, 0, processID);
		}
	}

	// send a MPI customized Datatype
	private void sendSpecialType(final int slaveID, final int dim, final int offset) throws MPIException 
	{
		final int blocklen[] 	= {1, dim - 1};
		final int disp[] 		= {0, offset + 1};
	 
		final Datatype mytype = Datatype.createIndexed(blocklen, disp, MPI.BYTE);
		mytype.commit();
	    	 
		plain1[0] = plain1[disp[1]-1];			// swap the first value to send to beginning			
		MPI.COMM_WORLD.send(plain1, 1, mytype, slaveID, slaveID);
	}

	
	private void gathering(	final int processID, final int numberProcesses) throws MPIException
	{	 
		// Master process
		if(processID == 0)
		{
			// Internal Master copy
			for(int i = 0; i < p_plain2.length; i++) 
			{
				plain2[i] = p_plain2[i];
			}
		
			final byte saveFirst = plain2[0];
		
			// Receiving data from the slaves
			for(int slaveID = 1; slaveID < numberProcesses-1; slaveID++)
			{	   
				recvSpecialType(slaveID, p_array_rows, slaveID * p_array_rows);
			}
			// Dealing with the last process (might have different chunk of data)
			if(numberProcesses > 1)
			{
				recvSpecialType(numberProcesses-1, rem_p_array_rows, (numberProcesses-1) * p_array_rows);
			}
		
			plain2[0] 	= saveFirst; 				// restore the original value
		}
		else // Slaves
		{
			MPI.COMM_WORLD.send		(p_plain2, p_array_rows, MPI.BYTE, 0, processID);
		}
}

	// send a MPI customized Datatype
	private void recvSpecialType(final int slaveID, final int dim, final int offset) throws MPIException 
	{
		final int blocklen[] 	= {1, dim - 1};
		final int disp[] 		= {0, offset + 1};
		...
		MPI.COMM_WORLD.recv(plain2, 1, mytype, slaveID, slaveID);
		plain2[disp[1]-1] = plain2[0];	// swap the first value to send to beginning
	}
...
void buildTestData() throws MPIException
{
    ...
    if(JGFCryptBench.rank==0) 
    {
      ...
      /* final int dim = ref_p_array_rows * MPI.COMM_WORLD.getSize(); // adjusted dim
    		plain1 = new byte [dim];
        crypt1 = new byte [dim];
        plain2 = new byte [dim];
        */
    ...
    /*
    p_plain1 = new byte [ref_p_array_rows];
    p_crypt1 = new byte [ref_p_array_rows];
    p_plain2 = new byte [ref_p_array_rows];*/
