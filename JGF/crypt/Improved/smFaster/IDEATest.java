...
class IDEATest
{
	...
	/**
	 * Bruno Medeiros : From the JGF original SM version the improvements are:
	 * 	> Creating of a single runnable per thread instead of two;
	 * 	> Calculation of the task distribution per thread only once, instead of two;
	 * 
	 */
	public final void Do()
	{		
		final int numSlaves = JGFCryptBench.nthreads -1; // minus slave
		final Thread team[] = new Thread [numSlaves];
		final int plainSize = plain1.length;
		
		// Set Timer
		final double start 	= System.currentTimeMillis();
		final int tslice 	= plainSize >> 3; 			// division by 8
		final int ttslice 	= (tslice +  numSlaves) / (numSlaves + 1);
		final int slice 		= ttslice << 3;				// multiply by 8	
		
		// Encrypt plain1.
		for(int i = 0; i < numSlaves; i++) 
		{
			final int slaveID = i + 1;
			team[i] = new Thread
			(
				(new Runnable()
				{
					public void run()
					{
						final int ilow 		= slaveID * slice;
						final int dim 		= ilow + slice;
						final int iupper 	= dim > plainSize ? plainSize : dim;
						
						cipher_idea(ilow, iupper, plain1, crypt1, Z);     	// Encrypt plain1.
						cipher_idea(ilow, iupper,crypt1, plain2, DK);    	// Decrypt.
					}
				})
			);
			team[i].start();
		...
		final int iupper = (slice > plainSize) ? plainSize : slice;
		
		// Master
		cipher_idea(0, iupper, plain1, crypt1, Z);     // Encrypt plain1.
		cipher_idea(0, iupper, crypt1, plain2, DK);    // Decrypt.
	...
	public final void buildTestData()
	{
	...
	private void cipher_idea(final int ilow, final int iupper, final byte [] text1, 
							final byte [] text2, final int [] key)
